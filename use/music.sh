...
init_lyricapi() {
    ...
    docker run -d \
        ...
        crpi-pqqbvdf8c8dv7tyr.cn-shanghai.personal.cr.aliyuncs.com/nas-mp/lyricapi:latest
}

init_music_tag_web() {
    echo "初始化 musictag2"
    mkdir -p "$DOCKER_ROOT_PATH/musictag2"
    curl -L https://naspt.oss-cn-shanghai.aliyuncs.com/music/naspt-musictag.tgz > "$CURRENT_DIR/naspt-musictag.tgz"
    ...
    docker run -d \
            ...
            --restart always \
            crpi-pqqbvdf8c8dv7tyr.cn-shanghai.personal.cr.aliyuncs.com/nas-mp/music_tag_web:latest
}

init_navidrome() {
    echo "初始化 navidrome"
    mkdir -p "$DOCKER_ROOT_PATH/navidrome"
    curl -L https://naspt.oss-cn-shanghai.aliyuncs.com/music/naspt-navidrome.tgz > "$CURRENT_DIR/naspt-navidrome.tgz"
    ...
    docker run -d \
        ...
        -v "$MUSIC_ROOT_PATH/links:/music" \
        crpi-pqqbvdf8c8dv7tyr.cn-shanghai.personal.cr.aliyuncs.com/nas-mp/navidrome:latest
}
